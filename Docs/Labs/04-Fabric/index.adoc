...
La base de Fabric es un lakehouse, creado sobre la capa de almacenamiento escalable de OneLake, y usa Spark y SQL para el procesamiento de datos. Un lakehouse es una plataforma unificada que combina:
...
La parte izquierda muestra un explorador para examinar las tablas y archivos del lakehouse. Las tablas se podrán consultar en SQL. Los archivos estarán almacenados en OneLake. Crear una carpeta `data` en la carpeta `Files` y subir un archivo https://raw.githubusercontent.com/MicrosoftLearning/dp-data/main/sales.csv[sales.csv] con datos de ejemplo.
...
[NOTE]
====
**Parquet** es un formato de _archivo de columnas_ de código abierto diseñado para almacenar datos de manera eficiente. A diferencia de los formatos tradicionales como CSV que almacenan datos fila por fila, Parquet organiza los datos por columnas, lo que permite leer solo las columnas necesarias para una consulta específica, reduciendo significativamente el tiempo de lectura y el uso de recursos. Además, Parquet aplica compresión automática de datos, reduciendo el espacio de almacenamiento hasta en un 75% comparado con CSV. Es especialmente útil en entornos de big data y análisis, siendo compatible con herramientas como Spark, y es el formato predeterminado para almacenar tablas Delta en lakehouses de Fabric. Por ejemplo, si tenemos un archivo CSV de 1 GB con datos de ventas y solo necesitamos analizar las columnas de fecha y total, Parquet nos permitirá leer únicamente esas columnas, mientras que con CSV tendríamos que leer todo el archivo.
====
...
Descargaremos en nuestro equipo este https://github.com/MicrosoftLearning/dp-data/raw/main/orders.zip[archivo .zip de ventas] y lo subiremos a una carpeta `orders` del lakehouse. El archivo contiene datos de ventas en formato CSV de los años 2019, 2020 y 2021. La figura siguiente muestra los tres archivos en la carpeta `orders` del exploradore del lakehouse.
